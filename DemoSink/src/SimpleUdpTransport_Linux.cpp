...
SimpleUdpTransport_Linux::SimpleUdpTransport() {
    fd = socket(AF_INET6, SOCK_DGRAM, 0);
    recv_thread = std::thread(&SimpleUdpTransport_Linux::SocketThread, this);
}
...
bool SimpleUdpTransport_Linux::SendRtp(const std::vector<uint8_t> &&data) {
...
bool SimpleUdpTransport_Linux::SendRtcp(const std::vector<uint8_t> &&data) {
    return SendRtp(std::move(data));
}

std::error_code SimpleUdpTransport_Linux::ListenOn(uint16_t port) {
...
void SimpleUdpTransport_Linux::SocketThread() {
    while (fd >= 0) {
        struct timeval tv;
        tv.tv_sec = 1;
        tv.tv_usec = 0;
...
void SimpleUdpTransport_Linux::ConnectTo(const sockaddr_storage &addr, const socklen_t addr_len) {
